...
char *
decode_text(unsigned char *in, int len)
{
    if(in == NULL || len == 8224) return "";
...
void
decode_external(Starling_entry *entr, unsigned char **var)
{
    if(entr->var_offset == 538976288 && entr->var_length == 8224){ // 20202020 and 2020 = empty
        entr->content = NULL;
    } else {
        entr->content = calloc(entr->var_length, sizeof(unsigned char));
        memcpy(entr->content, *var + entr->var_offset, entr->var_length);
    }
}

void
decode_all_text(Starling_db *db)
{
    int elen = 0;
    for(int i = 0; i < db->rec_ct; i++){
        for(int j = 0; j < db->hdr_ct; j++){
            if(db->recs[i].entries[j].decoded_content == NULL){
                ...
                db->recs[i].entries[j].decoded_content = decode_text(db->recs[i].entries[j].content, elen);
                db->recs[i].entries[j].true_length = (db->recs[i].entries[j].var_length == 8224 ? 0 : strlen(db->recs[i].entries[j].decoded_content));
            }
        }
    }
}
...
void
decode_all_external(Starling_db *db)
{
    for(int i = 0; i < db->rec_ct; i++){
        for(int j = 0; j < db->hdr_ct; j++){
            if(db->recs[i].entries[j].type == et_external){
                decode_external(&(db->recs[i].entries[j]), &(db->ext_entries));
            }
        }
    }
}
...
void
decode_all(Starling_db *db){
    if(db->ext_entries != NULL)
        decode_all_external(db);
    decode_all_text(db);
}
