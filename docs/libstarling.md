...
In addition, this header contains declarations of all of the functions offered by `libstarling`. Functions prefixed with `starling_parse_` are implemented in `dissect.c` and constitute the first stage in decoding a Starling database:
...
* `char *starling_decode_text(unsigned char *in, int len)`: translate Starling-encoded text into UTF-8.
